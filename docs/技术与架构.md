...
- 采用响应式设计基础 (PC/Tablet/Mobile 完美适配)
- 使用 `glass` 类实现磨砂玻璃质感设计
- **主题系统**: 支持 Light/Dark 模式切换，状态持久化
- **页面模块**: Home, Library, Reader, Details, Settings, Metadata, Libraries
- 通过 REST API 与后端通信，封装了统一的 `api` 客户端
...
- **路由模块**:
  ...
  - `/api/metadata` - 元数据管理
  - `/api/history` - 阅读历史
  - `/api/config` - 配置管理 (Env/Json)
  - `/api/ratings` - 评分系统
  - `/api/image` - 图片代理服务 (解决绝对路径跨域/安全问题)
...
|------|------|------|
| id | INTEGER | 主键，自增 |
| manga_id | INTEGER | 漫画 ID (Foreign Key) |
| rating | REAL | 评分值（0-10） |
| comment | TEXT | 评论内容 |
| created_at | DATETIME | 创建时间 |
| updated_at | DATETIME | 更新时间 |

**注意**: 漫画表中的 `rating` 和 `rating_count` 字段会根据评分表自动计算和更新。
